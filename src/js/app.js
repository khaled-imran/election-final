...
App = {
  ...
  initWeb3: function() {
    // Todo: for public demo
    // var network_endpoint = 'https://k0y6vftf8s:2pi3-y80uCubwqt69rStvkp-PD8elgdYSHAqZwvBzlk@' + rpc_endpoint;
    ...
    App.contracts.Election.deployed().then(function(instance) {
      ...
      for (var i = 1; i <= candidatesCount; i++) {
        electionInstance.candidates(i, { from: App.account }).then(function(candidate) {
          ...
          var candidateTemplate = "<tr id='candidate_"+ id +"'><th>" + id + "</th><td>" + name + "</td><td>" + voteCount + "</td></tr>"
          if(candidatesResults.find('#candidate_' + id).length){
            candidatesResults.find('#candidate_' + id).replaceWith(candidateTemplate);
          } else {
